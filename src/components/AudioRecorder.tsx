...
export function AudioRecorder({ value, onChange }: AudioRecorderProps) {
  ...
  // Si el usuario navega hacia atras mientras graba, se libera el microfono al desmontar.
  useEffect(() => {
    return () => {
      if (audioRecorder.isRecording) {
        audioRecorder.stop().catch(() => {
          // El componente ya se esta desmontando; no hay UI a la cual reportar el error.
        });
      }
    };
  }, [audioRecorder]);
    ...
    try {
      ...
      const recordingError =
        error instanceof AudioRecordingError
          ? error
          : new AudioRecordingError('recording-failed', 'No se pudo iniciar la grabacion');
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      if (!uri) {
        throw new AudioRecordingError('recording-failed', 'No pudimos obtener la nota grabada');
      }
      ...
      const recordingError =
        error instanceof AudioRecordingError
          ? error
          : new AudioRecordingError('recording-failed', 'No pudimos detener la grabacion');
  ...
  if (status.kind === 'recording') {
    return (
      <View
        style={[styles.card, { backgroundColor: colors.surfaceSecondary, borderColor: colors.danger }]}>
        <View style={styles.recordingRow}>
          <View style={[styles.recordingDot, { backgroundColor: colors.danger }]} />
          <Text style={[styles.recordingLabel, { color: colors.text }]}>Grabando</Text>
    ...
    return (
      <View
        ...
        <View style={styles.errorActions}>
          ...
          {status.error.kind === 'permission-denied' && !status.error.canAskAgain ? (
            <Pressable
  ...
  if (value) {
    return (
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.text }]}>Nota grabada</Text>
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={styles.titleRow}>
        <Text style={[styles.cardTitle, { color: colors.text }]}>Nota de voz</Text>
        <View style={[styles.optionalBadge, { backgroundColor: colors.surfaceSecondary }]}>
          <Text style={[styles.optionalBadgeText, { color: colors.textSecondary }]}>Opcional</Text>
        </View>
      </View>
